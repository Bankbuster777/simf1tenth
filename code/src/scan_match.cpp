  ...
  public:
    ...
    void handleLaserScan(const sensor_msgs::LaserScan::ConstPtr& msg) {
      ...
      while (count < MAX_ITER && (curr_trans != prev_trans || count==0)) {
        // transform(curr_trans) points to transformed_points
        ...
        cout << "Correspond Error: " << correspond_error(corresponds) << endl;
    ...
    double correspond_error(vector<Correspondence>& c){
      double error_x = 0, error_y = 0;
      const int n = c.size();

      for(int i = 0; i < n; ++i){
        error_x = error_x + (c[i].pix - c[i].pj1->getX())*(c[i].pix - c[i].pj1->getX());
        error_y = error_y + (c[i].piy - c[i].pj1->getY())*(c[i].piy - c[i].pj1->getY());
      }
    ...
    void publishPos() {
     ...
     tf3d.setValue(static_cast<double>(global_tf(0,0)), static_cast<double>(global_tf(0,1)), 0,
                   static_cast<double>(global_tf(1,0)), static_cast<double>(global_tf(1,1)), 0,
                   0,                                   0,                                   1);
